...
static bool g_enable_16kb_app_compat = false;
...
bool ElfReader::IsEligibleFor16KiBAppCompat(ElfW(Addr) * vaddr) {
  ...
  if (!HasAtMostOneRelroSegment(&relro_phdr)) {
    add_dlwarning(name_.c_str(), "\"%s\": Compat loading failed: Multiple RELRO segments found", name_.c_str());
    return false;
  }
  ...
  for (size_t i = 0; i < phdr_num_; ++i) {
    ...
    if ((prot & PROT_WRITE) && (prot & PROT_READ)) {
      ...
      if (last_rw && last_rw != prev) {
        add_dlwarning(name_.c_str(), "\"%s\": Compat loading failed: ELF contains multiple non-adjacent RW segments",
                      name_.c_str());
        return false;
      }
  ...
  if (!segment_contains_prefix(first_rw, relro_phdr)) {
    add_dlwarning(name_.c_str(), "\"%s\": Compat loading failed: RELRO is not in the first RW segment", name_.c_str());
    return false;
  }

  uint64_t end;
  if (__builtin_add_overflow(relro_phdr->p_vaddr, relro_phdr->p_memsz, &end)) {
    add_dlwarning(name_.c_str(), "\"%s\": Compat loading failed: relro vaddr + memsz overflowed", name_.c_str());
    return false;
  }
...
bool ElfReader::CompatMapSegment(size_t seg_idx, size_t len) {
  ...
  if (TEMP_FAILURE_RETRY(pread64(fd_, start, len, offset)) == -1) {
    LOGE("Compat loading: \"%s\" failed to read LOAD segment %zu: %m", name_.c_str(), seg_idx);
    return false;
  }
