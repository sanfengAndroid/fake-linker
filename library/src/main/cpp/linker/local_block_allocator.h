...
public:
  static T *alloc() { return reinterpret_cast<T *>(LocalSizeBasedAllocator<sizeof(T)>::alloc()); }

  static void free(T *ptr) { LocalSizeBasedAllocator<sizeof(T)>::free(ptr); }

  static void purge() { LocalSizeBasedAllocator<sizeof(T)>::purge(); }
};
