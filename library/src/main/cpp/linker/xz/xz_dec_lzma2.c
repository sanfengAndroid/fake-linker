...
static inline bool dict_has_space(const struct dictionary *dict) { return dict->pos < dict->limit; }
/*
...
static inline bool rc_limit_exceeded(const struct rc_dec *rc) { return rc->in_pos > rc->in_limit; }
/*
 * Return true if it is possible (from point of view of range decoder) that
 * we have reached the end of the LZMA chunk.
 */
static inline bool rc_is_finished(const struct rc_dec *rc) { return rc->code == 0; }
...
static void lzma_reset(struct xz_dec_lzma2 *s) {
  ...
  for (i = 0; i < PROBS_TOTAL; ++i) probs[i] = RC_BIT_MODEL_TOTAL / 2;
  rc_reset(&s->rc);
}
/*
