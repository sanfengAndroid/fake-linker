 ...
 */
static inline int bcj_x86_test_msbyte(uint8_t b) { return b == 0x00 || b == 0xFF; }
...
static size_t bcj_ia64(struct xz_dec_bcj *s, uint8_t *buf, size_t size) {
  ...
  for (i = 0; i < size; i += 16) {
    ...
    for (slot = 0, bit_pos = 5; slot < 3; ++slot, bit_pos += 41) {
      ...
      bit_res = bit_pos & 7;
      instr = 0;
      for (j = 0; j < 6; ++j) instr |= (uint64_t)(buf[i + j + byte_pos]) << (8 * j);
      norm = instr >> bit_res;
      if (((norm >> 37) & 0x0F) == 0x05 && ((norm >> 9) & 0x07) == 0) {
        ...
        instr &= (1 << bit_res) - 1;
        instr |= norm << bit_res;
        for (j = 0; j < 6; j++) buf[i + j + byte_pos] = (uint8_t)(instr >> (8 * j));
      }
    }
  }
  return i;
...
static size_t bcj_armthumb(struct xz_dec_bcj *s, uint8_t *buf, size_t size) {
  ...
  for (i = 0; i <= size; i += 2) {
    if ((buf[i + 1] & 0xF8) == 0xF0 && (buf[i + 3] & 0xF8) == 0xF8) {
      addr = (((uint32_t)buf[i + 1] & 0x07) << 19) | ((uint32_t)buf[i] << 11) | (((uint32_t)buf[i + 3] & 0x07) << 8) |
        (uint32_t)buf[i + 2];
      addr <<= 1;
