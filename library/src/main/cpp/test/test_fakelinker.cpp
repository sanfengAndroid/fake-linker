...
static void Init() {
  soinfo::Init();
  std::cout << "init soinfo completed" << std::endl;
  android_namespace_t::Init();
  std::cout << "init android namespace completed" << std::endl;
...
static void init_env() {
  ...
  std::cout << "android version: " << android_api << std::endl;
...
void print_soinfo_offset() {
  ...
  std::cout << "type: " << name << ", arch: " << get_arch_name() << std::endl;
  if constexpr (has_member_soname_<T>::value) {
    std::cout << "\tsoname offset: " << offsetof(T, soname_) << std::endl;
  }

  std::cout << "\tchildren_ offset: " << offsetof(T, children_) << std::endl;
  std::cout << "\tparents_ offset: " << offsetof(T, parents_) << std::endl;
  std::cout << "\tst_dev_ offset: " << offsetof(T, st_dev_) << std::endl;
  std::cout << "\tst_ino_ offset: " << offsetof(T, st_ino_) << std::endl;

  if constexpr (has_member_ld_library_paths_<T>::value) {
    std::cout << "\tandroid_namespace ld_library_paths_ offset: " << offsetof(T, ld_library_paths_) << std::endl;
  }
  if constexpr (has_member_default_library_paths_<T>::value) {
    std::cout << "\tandroid_namespace default_library_paths_ offset: " << offsetof(T, default_library_paths_)
              << std::endl;
  }
  if constexpr (has_member_permitted_paths_<T>::value) {
    std::cout << "\tandroid_namespace permitted_paths_ offset: " << offsetof(T, permitted_paths_) << std::endl;
  }

  if constexpr (has_member_whitelisted_libs_<T>::value) {
    std::cout << "\tandroid_namespace whitelisted_libs_ offset: " << offsetof(T, whitelisted_libs_) << std::endl;
  }
}
...
TEST(FakeLinker, relocateTest) {
  ...
  const char *test_library = "libssl.so";
  const char *test_function = "gettimeofday";
  void *handle = dlopen(test_library, RTLD_NOW);
  ASSERT_TRUE(handle) << "dlopen " << test_library;
  auto test_soinfo = g_fakelinker_export.soinfo_find(SoinfoFindType::kSTName, test_library, nullptr);
  ASSERT_TRUE(test_soinfo) << "find " << test_library << " soinfo";
  ...
  handle = dlopen(test_library, RTLD_NOW);
  ASSERT_TRUE(handle) << "dlopen local " << test_library;
  test_soinfo = g_fakelinker_export.soinfo_find(SoinfoFindType::kSTName, test_library, nullptr);
  ASSERT_TRUE(test_soinfo) << "find " << test_library << " soinfo 2";
  ...
  if (android_api < __ANDROID_API_M__) {
    EXPECT_EQ(*test_import_function, &gettimeofday) << "call_manual_relocation_by_soinfo verify";
  } else {
