...
public:
  ScopedUtfChars(JNIEnv *env, jstring s, bool check_null = true) : env_(env), string_(s) {
    if (s == nullptr) {
      utf_chars_ = nullptr;
      if (check_null) {
  ...
  ScopedUtfChars(ScopedUtfChars &&rhs) : env_(rhs.env_), string_(rhs.string_), utf_chars_(rhs.utf_chars_) {
    rhs.env_ = nullptr;
    rhs.string_ = nullptr;
